...
function CountryLabel({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/80 px-4 py-3 text-white shadow-[0_10px_40px_rgba(0,0,0,0.45)] backdrop-blur-xl">
    ...
    >
      ...
      {locations.map((location) => (
        <Marker key={location.name} coordinates={location.coordinates}>
          ...
          <circle r={14} fill="rgba(239,68,68,0.18)" />
      ...
      <Marker coordinates={figueres.coordinates}>
        ...
        <circle r={22} fill="rgba(239,68,68,0.22)" />

        <foreignObject x={18} y={-8} width={210} height={78}>
          <div className="rounded-2xl border border-red-500/30 bg-black/80 px-5 py-4 text-white shadow-[0_10px_50px_rgba(239,68,68,0.22)] backdrop-blur-xl">
...
export default function EuropeMap() {
  return (
    <>
      {/* DESKTOP */}
      <div className="relative hidden h-[560px] overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#050505] p-7 shadow-[0_30px_120px_rgba(0,0,0,0.65)] lg:block">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_68%,rgba(239,68,68,0.26),transparent_34%),radial-gradient(circle_at_45%_35%,rgba(255,255,255,0.08),transparent_42%)]" />
        ...
        <div className="absolute left-7 right-7 top-24 z-10 h-[330px] overflow-hidden rounded-[2rem] border border-white/10 bg-black/35">
          <MapBase />
        </div>

        <div className="absolute bottom-7 left-7 right-7 z-20 grid gap-4 md:grid-cols-3">
      ...
      {/* MOBILE: sin mapa para evitar condensación */}
